...
((w, d, $) => {
     ...
     *
     * @example
     * stickyElement('.js-element', '.js-element-space');
     * @param {string} [element=.js-element]
     * @param {string} [space=.js-element-space]
     * @returns {void}
     */
    const stickyElement = (element = '.js-element', space = '.js-element-space') => {
        const el = d.querySelector(element);
        const elSpace = d.querySelector(space);
        ...
        if (w.innerWidth > deviceWidth) {
            w.addEventListener('scroll', () => {
                ...
                if (scrollTop > elOffsetTop && !hasClass) {
                    elSpace.style.height = `${elHeight}px`;
